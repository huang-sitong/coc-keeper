...
import keeper.base.entities as platform_entities
import keeper.base.event_logger as abstract_platform_logger
...
class TelegramAdapter(abstract_platform_adapter.AbstractMessagePlatformAdapter):
    ...
    def __init__(self, config: dict, logger: abstract_platform_logger.AbstractEventLogger):
        ...
        async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
            # coc-keeper 适配：暂不处理 LangBot 流水线表单回调，仅应答并记录。
            query = update.callback_query
            await query.answer()
            await self.logger.warning(
                f"Telegram callback query ignored (form actions not supported in coc-keeper yet): {query.data!r}"
            )
